...
| `QD-008` | 연락처와 견적·동의를 단계형 모달에서 받은 뒤 결과를 표시하되 계산 상태와 제출 상태는 독립적으로 관리한다. | 채택 | [전달된 UI 흐름](quick-estimate-ui-design-plan.md)을 따르면서 저장 실패를 결과나 접수 성공으로 오인하지 않는다. |
