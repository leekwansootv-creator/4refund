...
> 2026-09-04 RF-PR3: 선조회 → 선택적 상세 견적 상담 신청 흐름을 milestone에 구현했다. 운영 공개 전이며 실제 검증·병합 상태는 [실행 기록](quick-estimate-result-first-pr-roadmap.md)을 따른다. 기존 Figma는 시각 자산의 기준이고 새 순서의 승인 증거가 아니다.
...
현재 문서는 기존 시각 자산과 RF-PR3의 구현 계약을 설명한다. 새 화면은 동일 모달 안에서 전환하고 운영 endpoint·산식·고지 계약은 유지한다.
...
stateDiagram-v2
    [*] --> Estimate: 환급액 조회하기
    Estimate --> Result: 브라우저 계산
    Result --> Estimate: 다시 조회하기
    Result --> Contact: 상세 견적 받기
    Contact --> Result: 결과로 돌아가기
    Contact --> Submitting: 최종 신청과 검증
    Submitting --> Complete: 저장 성공 응답
    Submitting --> Failed: 실패 또는 미확인
    Failed --> Submitting: 동일 payload 재시도
    Complete --> [*]: 확인
```

### 조회 조건과 결과

업종은 기존 KSIC 21개와 표시 순서, 직원 수는 1~6,000명 정수를 유지한다. 연락처·동의 없이 조회하며 난수를 한 번 생성해 계산한 결과만 확정한다. requestId 생성이나 접수 전송은 없다.

결과는 업종·직원 수·참고용 예상액 → 상담 안내 → 주요 `상세 견적 받기` → 보조 `다시 조회하기` 순서다. 재조회 조건을 편집하는 동안 기존 결과는 제출 대상으로 쓰지 않는다. endpoint 미설정 시 결과는 표시하되 신청 입력 대신 문의 영역으로 이동한다.

### 상세 견적 신청 정보

같은 모달에서 상담 설명과 기존 견적 요약 다음에 회사명(1–100자), 담당자 이름(1–50자), 이메일(최대 254자), 전화번호(정규화 후 9~11자리)를 입력한다. 필드 이탈 후 오류 설명을 연결한다. 두 동의 전문과 버전은 유지하며 개인정보 필수·마케팅 선택이다.

`상세 견적 신청하기`는 연락처와 필수 동의가 유효할 때만 활성화된다. 이 버튼에서만 모달 진입 이후 체류 시간·허니팟·최종 payload를 검증하고 요청 ID를 생성해 제출한다. `결과로 돌아가기`는 연락처·동의·예상액을 보존하며 전송하지 않는다. 상세 견적은 상담을 통한 후속 안내로, 정밀 금액 자동 계산을 뜻하지 않는다.

### 접수 진행·완료·실패

- 전송 중 폼·이전 이동·닫기를 제거하거나 차단하고 진행 안내를 표시한다.
- 저장 성공 응답 후에만 접수 완료와 `확인`을 표시한다. 같은 모달에서 성공 요청 재제출은 불가능하다.
- 저장 실패·요청 제한은 동일 payload로 재시도한다. 입력·동의 검증 거절은 같은 견적으로 수정 신청하며 새 ID를 사용한다. 규칙 거절은 새 결과 확인을 먼저 요구한다.
- 네트워크·시간 초과·판독 불가·내부 오류는 저장 여부 미확인으로 안내하고 같은 요청만 재시도한다. 이후 재시도가 거절되어도 최초 불확실성을 유지한다.
- 미확인 상태의 닫기에는 중복 가능성과 요청 복원 불가 경고를 표시한다. 명시적 닫기 선택 전까지 모달을 유지한다.
- 3초 미만은 입력 확인 후 재신청, 2시간 초과는 새 시작을 안내한다. 시간을 임의 보정하지 않는다.
- 재진입 시 상태를 초기화하고 Web Storage에는 저장하지 않는다. 모달 세션을 넘는 중복 방지는 보장하지 않는다.
...
| 이메일 field 없음                         | 필수 제출 계약에 `email` 존재                                | 신청 정보 단계에 이메일 field 추가                                           |
...
Figma에 없는 제출 중, 저장 실패, 재시도, mobile overflow 상태는 기존 제출 계약과 접근성 요구를 바탕으로 추가한다. 이를 가짜 성공이나 새로운 제품 단계로 취급하지 않고 접수 상태 전용 화면으로 구현한다.
...
- 연락처와 필수 동의가 모두 유효할 때만 `상세 견적 신청하기` 활성화
- 업종과 직원 수만으로 `조회하기` 활성화
- 마케팅 미동의 상태에서 상담 신청 허용
...
- CTA에서 조건, 결과, 신청 정보·동의, 접수 완료까지 정상 흐름
- 조회·재조회·신청 진입·입력에서는 외부 요청이 없고 최종 신청에서만 제출
- 조회 event당 난수 한 번, 새 신청 event당 request ID 한 번 생성
- 저장 성공 전 접수 성공 문구를 표시하지 않음
- timeout·network·판독 불가 후 같은 payload로 재시도
- 새 조회에서는 새 난수를 사용하며 request ID는 신청할 때 생성
