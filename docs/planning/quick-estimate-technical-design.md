...
> 2026-09-04 RF-PR3: [선조회 플로우](quick-estimate-result-first-flow.md)의 계산·제출 분리를 milestone에 구현했다. 공개 전이며 최종 제출·계산·저장 계약은 유지한다.
...
| `QD-008` | 업종·직원 수로 먼저 결과를 표시하고 최종 상담 신청에서만 연락처·동의를 검증해 제출한다.               | 채택 | [전달된 UI 흐름](quick-estimate-ui-design-plan.md)을 따르면서 저장 실패를 결과나 접수 성공으로 오인하지 않는다. |
...
RF-PR3는 조회 조건·결과·신청 정보·접수 상태 화면을 구분한다. 화면 이동이나 필드 편집은 전송하지 않는다. 결과는 조회 이벤트에서 확정하며 신청에는 해당 결과를 그대로 사용한다. 입력·동의 거절 뒤 수정 요청만 새 ID를 만들고, 저장 미확인 상태는 후속 거절에도 불확실성을 유지해 동일 요청 재시도만 허용한다. 닫기 경고와 재진입 초기화의 한계는 UI 기획을 따른다.
...
- 운영 접근자는 `QD-010` 소유 계정과 2026-09-04 승인된 기존 편집자 3명으로 제한한다. [운영 runbook](../operations/quick-estimate-runbook.md#2026-09-04-접근-기준-승인)의 기존 접근 유지 범위를 따르며 공개 링크 공유와 별도 export·backup을 금지한다.
...
| Sheet 공유 노출     | 공개 링크 금지, 소유 계정과 승인된 기존 편집자 3명만 접근, 다중 인증, 접근자 변경 시 재승인 |
...
- 기능 공개 후 장애가 발생하면 이전 `HeroSection`과 독립 `RefundCasesSection` 조합으로 코드 롤백한다.
