...
- 구현·배포 상태: RF-PR3에서 새 흐름을 구현했다. 운영 공개 전이며 검사·병합·실제 저장 여부는 실행 기록을 따른다.
...
기존 문서는 연락처를 먼저 받는 구현 기준과 과거 출시 계획을 담고 있다. 이 문서는 그중 화면 순서, 동의 배치, 제출 시점과 결과 후속 동작의 변경안을 정의한다. RF-PR3에서 해당 항목의 기존 문서 본문을 이 계약에 맞춰 갱신했다. 계산 규칙, 최종 제출 데이터, 저장·운영 계약은 별도 변경이 명시되지 않는 한 기존 문서를 따른다.
...
아래는 현재 파일을 기준으로 한 후속 구현 범위다. RF-PR3의 구현 대상과 재사용 경계다.
