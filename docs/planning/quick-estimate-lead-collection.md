...
> 2026-09-04 RF-PR3: [선조회 플로우](quick-estimate-result-first-flow.md)를 milestone에 구현했다. 운영 공개 전이며 실제 저장 검증과 RF-PR5를 공개 조건으로 유지한다.
...
| 화면 구조      | 루트 랜딩의 기존 hero를 견적 hero로 교체하고, hero 버튼이 같은 페이지의 단계형 모달을 열어 업종·직원 수만으로 결과를 표시하고 원하는 사용자만 연락처·동의를 입력해 상세 견적 상담을 신청한다. |
...
- 첫 단계는 업종·직원 수이며, 결과에서 상세 견적 받기를 선택한 뒤 연락처·동의를 입력한다.
- `조회하기`는 예상 결과만 표시한다. 최종 `상세 견적 신청하기` 실행 전에는 리드·알림을 생성하지 않는다.
...
- Figma의 `직함`을 수집하지 않고 승인된 `회사명`과 누락된 `이메일`을 신청 정보 단계에 반영한다.
...
- Google Sheet는 승인된 이관수 소유 계정과 2026-09-04 승인된 기존 편집자 3명에게만 공유한다. 상담 원본 열람을 포함하는 기존 접근 유지 승인과 재승인 범위는 [운영 runbook](../operations/quick-estimate-runbook.md#2026-09-04-접근-기준-승인)을 따른다.
