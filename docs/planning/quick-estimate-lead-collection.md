...
데스크톱 페이지, 컴포넌트, 단계형 모달과 결과 Figma는 전달되었으며 [UI 디자인 구현 기획](quick-estimate-ui-design-plan.md)에서 기존 데이터 계약과의 차이를 정리했다. 별도 모바일 node와 계약에 맞게 보정한 공개 문구는 아직 확인이 필요하므로 이 문서 변경에서는 프런트엔드 코드를 구현하지 않는다.
...
| 화면 구조      | 메인 CTA에서 `/quick-estimate`로 이동하고, 연락처와 견적·동의를 단계형 모달로 받은 뒤 결과를 표시한다.                                                   |
| 현재 작업 범위 | 전달된 Figma와 기존 계약의 정합성을 문서화하며 프런트엔드는 구현하지 않는다.                                                                             |

### 전달된 디자인으로 확정한 사항

- 메인 랜딩 hero에 별도 견적 CTA를 두고 `/quick-estimate`로 이동한다.
- 별도 페이지의 hero CTA가 단계형 모달을 연다.
- 첫 단계에서 연락처, 두 번째 단계에서 업종·직원 수와 동의를 입력한다.
- `조회하기` 이후 예상 결과를 표시한다.
- 약관 전문은 모달 안에서 펼치고 접는다.
- 메인 CTA의 실제 공개 연결은 전체 저장 흐름과 출시 QA가 끝난 PR에서 수행한다.
...
기본·입력 완료·결과와 component variant는 전달되었다. 제출 중, 접수 실패·재시도와 별도 mobile node는 없으므로 구현 기획에 정의한 상태와 파생 반응형 규칙을 승인한 뒤 UI를 시작한다.
...
- [ ] 별도 mobile Figma 또는 파생 반응형 규칙 승인
- [ ] 승인된 수집 항목·1년 보유 계약을 반영한 사용자 문구 최종 승인
