...
현재 운영 Sheet에는 사람용 `상담 목록`이 구현되어 있지 않다. 후속 구현과 접근 계정 승인이 끝나기 전에는 상담 담당자에게 이관수 계정 자격 증명을 공유하거나 기존 `leads`를 임의로 재구성하지 않는다. 상담 목록 구현 후에도 원본 24개 컬럼은 수정하지 않고 상담 담당자는 한글 업무 화면에서만 허용된 운영값을 변경한다.
