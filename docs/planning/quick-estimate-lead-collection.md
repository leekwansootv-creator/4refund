...
메인 랜딩 교체안, 컴포넌트, 단계형 모달과 결과 Figma는 전달되었으며 [UI 디자인 구현 기획](quick-estimate-ui-design-plan.md)에서 기존 데이터 계약과의 차이를 정리했다. 별도 모바일 node와 계약에 맞게 보정한 공개 문구는 아직 확인이 필요하므로 이 문서 변경에서는 프런트엔드 코드를 구현하지 않는다.
